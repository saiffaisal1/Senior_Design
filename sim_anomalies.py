...
for idx in point_indices:
    volt_spike = np.random.choice([+120, -120])
    freq_spike = np.random.choice([+5, -5])
...
for _ in range(num_clustered_anomalies):
    ...
    for i in range(cluster_len):
        idx = start_idx + i
        volt_spike = np.random.choice([+100, -100])
        freq_spike = np.random.choice([+4, -4])
        df.at[idx, voltage_col] += volt_spike
        df.at[idx, frequency_col] += freq_spike
        df.at[idx, 'label'] = 1
...
print(f"Anomaly-injected data saved to: {output_file}")
print(f"Total injected anomalies: {df['label'].sum()}")